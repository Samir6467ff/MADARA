...
handler.command = /^(facebook|fb|فيس|فيسبوك|فيس_بوك|fb2|facebookdl2|fbdl2|facebook3|fb3|facebookdl3|fbdl3|facebook4|fb4|facebookdl4|fbdl4|facebook5|fb5|facebookdl5|fbdl5)$/i
//handler.limit = 3 
...
if (!args[0]) return conn.reply(m.chat, `${lenguajeGB['smsAvisoMG']()}*مش كده يا حمار ❌*\n*اهو بص كده افهم بقا 💯*\n*${usedPrefix + command} https://www.facebook.com/watch?v=636541475139`, fkontak, m)
if (!args[0].match(/www.facebook.com|fb.watch/g)) return conn.reply(m.chat, `${lenguajeGB['smsAvisoMG']()}*مش كده يا حمار ❌*\n*اهو بص كده افهم بقا 💯*\n*${usedPrefix + command} https://www.facebook.com/watch?v=636541475139*`, fkontak, m)
try {
await conn.reply(m.chat, `${lenguajeGB['smsAvisoEG']()}*جار تحميل المقطع لك ياورع اصبر 🐤🚀*`, fkontak, m)
let Rres = await fetch(`https://api.lolhuman.xyz/api/facebook?apikey=${lolkeysapi}&url=${args[0]}`)
let Jjson = await Rres.json()
let VIDEO = Jjson.result[0]
if (VIDEO == '' || !VIDEO || VIDEO == null) VIDEO = Jjson.result[1]
conn.sendFile(m.chat, VIDEO, 'error.mp4', `✅ *تمت العملية بنجاح يلا طير*\n${wm}`, m)    
} catch (err1) {
console.log('1 ' + err1)    
try {
let ress = await fg.fbdl(args[0])
let urll = await ress.data[0].url    
await conn.sendFile(m.chat, urll, 'error.mp4', '✅ تمت العملية بنجاح يلا طير\n${wm}', m)     
} catch (err2) {
console.log('2 ' + err2)    
try {
...
let ur = result.url    
await conn.sendFile(m.chat, ur, 'error.mp4', '✅ *تمت العملية بنجاح يلا طير*\n${wm}', m)}
} catch (err3) {
console.log('3 ' + err3)    
try { 
...
await conn.sendFile(m.chat, videovio, `error.mp4`, '✅ *تمت العملية بنجاح يلا طير*\n${wm}', m)
} catch (err4) {
console.log('4 ' + err4)    
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', '✅ *تمت العملية بنجاح يلا طير*\n${wm}', m)         
} catch (err5) {
console.log('5 ' + err5)    
try {
const { result } = await facebookdl(args[0]).catch(async _ => await facebookdlv2(args[0])).catch(async _ => await savefrom(args[0]))
for (const { url, isVideo } of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '✅ *تمت العملية بنجاح يلا طير*\n${wm}', m)    
} catch (err6) {
console.log('6 ' + err6)    
